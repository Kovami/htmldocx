...
| TipTap | `StarterKit` вместе с `TableKit`, `TextStyleKit`, `TextAlign`, `Image` (`inline: true`, `allowBase64: true`), `Subscript`, `Superscript`, `Highlight`, `Mathematics` — и расширение, которое сохраняет `style` и `id` у блоков и картинок (`addGlobalAttributes`, среди типов — `image`): без него TipTap их выбрасывает |
...
- редактор, который не моделирует какое-то форматирование, выбрасывает его ещё до библиотеки: TipTap не хранит межбуквенный интервал (и `style` у картинок без расширения выше), TinyMCE — табуляции внутри блоков кода.
