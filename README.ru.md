...
**Базовая типографика.** Текст, у которого нет своего форматирования, каждый редактор показывает шрифтом из своего content-стиля — профиль исходит ровно из него, поэтому документ, набранный в редакторе, в Word выглядит так же: у CKEditor это Helvetica браузерного размера medium с интервалом 1.5, у TinyMCE — системный шрифт с 1.4, у SunEditor 3 — Helvetica Neue 16px цветом #333 с интервалом 1.5. Для plain HTML это Calibri 11pt. Стек TinyMCE начинается с «системного шрифта этой платформы», а DOCX так попросить не может: Word получает Segoe UI — его и показывает Windows, — тогда как macOS показывает San Francisco с другими метриками, и текст там выходит длиннее или короче. Если приложение оформляет редактор иначе — а так обычно и есть — или его пользователи на Mac, скажите об этом:
...
Родовое семейство — `monospace`, `sans-serif`, `serif` — попадает в Word как Courier New, Arial или Times New Roman, а межстрочные интервалы считаются так, как его рисует Chromium на macOS: шрифтами Courier, Helvetica или Times.

**Настройка редактора.** Редакторы выбрасывают всё, чего нет в их схеме: из коробки CKEditor, TipTap и SunEditor сохраняют от четверти до половины написанного библиотекой форматирования (TinyMCE — почти всё). С плагинами и белыми списками ниже они сохраняют 97,9–99,7%, и то, что они отдают обратно, печатается с 96,1–97,3% краски на своём месте (см. бенч ниже). Точные конфигурации, на которых считается бенч, лежат в [`bench/fidelity/editors`](bench/fidelity/editors).
...
| TipTap | `StarterKit` вместе с `TableKit`, `TextStyleKit`, `TextAlign`, `Image` (`inline: true`, `allowBase64: true`), `Subscript`, `Superscript`, `Highlight`, `Mathematics` — и расширение, которое сохраняет `style` и `id` у блоков, картинок и индексов, а межбуквенный интервал — у `textStyle` (`addGlobalAttributes`): без него TipTap их выбрасывает |
| SunEditor 3 | Все его плагины, KaTeX в виде `externalLibs: { katex: { src: katex } }`, `elementWhitelist: 'section\|colgroup\|col'`, `attributeWhitelist: { '*': 'style\|id\|role\|start\|value\|data-[^\\s]+' }` и `strictMode` с `attrFilter: false` и `styleFilter: false` (остальные его фильтры включены) |
...
Plain HTML и SunEditor настраиваются в первую очередь, остальные редакторы — следом.
...
| --- | ---: | ---: | ---: | ---: | ---: |
| Текст и форматирование знаков | 94,3% | 92,8% | 92,2% | 94,2% | 92,8% |
| Шрифты и типографика | 98,1% | 98,1% | 98,2% | 98,1% | 98,2% |
| Списки | 96,2% | 96,2% | 96,2% | 96,2% | 96,3% |
| Таблицы | 92,4% | 92,4% | 92,4% | 92,4% | 92,4% |
| Изображения | 100,0% | 100,0% | 100,0% | 100,0% | 100,0% |
| Отчёт на шесть страниц | 97,7% | 97,7% | 97,7% | 97,7% | 97,7% |
| Сноски | 100,0% | 100,0% | 100,0% | 100,0% | 100,0% |
| Колонтитулы, комментарии | 100,0% | 100,0% | 100,0% | 100,0% | 100,0% |
| **Среднее** | **97,3%** | **97,2%** | **97,1%** | **97,3%** | **97,2%** |

**HTML → DOCX**: собственный HTML редактора в Chromium с его стилями контента против того, как Word печатает DOCX, который из него пишет библиотека.

| Документ | Plain HTML | SunEditor | CKEditor | TinyMCE | TipTap |
| --- | ---: | ---: | ---: | ---: | ---: |
| Текст и форматирование знаков | 97,3% | 96,7% | 97,0% | 19,5% | 97,3% |
| Списки | 100,0% | 100,0% | 100,0% | 31,7% | 100,0% |
| Таблицы | 99,6% | 98,5% | 98,8% | 59,1% | 98,2% |
| Изображения | 99,4% | 99,0% | 98,2% | 69,1% | 99,4% |
| **Среднее** | **99,1%** | **98,5%** | **98,5%** | **44,8%** | **98,7%** |
<!-- bench:end -->
...
| `b`, `strong`, `i`, `em`, `u`, `s`, `del`, `sup`, `sub`, `mark`, `small`, `code`, `q`, … | Форматирование знаков: насыщенность, начертание, подчёркивание (вместе с его типом), зачёркивание, положение, цвет, выделение, шрифт, размер, межбуквенный интервал, прописные и капитель, тень и кернинг |
...
| Форматирование знаков | `<strong>`, `<em>`, `<u>`, `<del>`, `<sup>`, `<sub>` внутри `<span>` со шрифтом, размером, цветом, выделением и тенью; кернинг только там, где его делает Word |
...
| Разрывы строк и страниц, табуляции | `<br>`, `page-break-before`, сохранённые пробелы; в абзаце с отступом текст после табуляции встаёт на позиции Word, которые отсчитываются от поля |
...
- редактор, который не моделирует какое-то форматирование, выбрасывает его ещё до библиотеки: TipTap без расширения выше не хранит `style` у картинок и индексов и межбуквенный интервал, TinyMCE — табуляции внутри блоков кода.
