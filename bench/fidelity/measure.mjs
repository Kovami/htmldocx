// What the fidelity benches share: printing HTML the way a Word document asks
// to be printed, and comparing the result with the PDF Word printed.
...
/**
 * Prints `html` (a full document) at the page size `page` gives, compares it
 * with Word's PDF and returns the scores; `dir` receives the HTML, the PDF
 * and the page images, named after `name`.
 */
export async function measure(browser, html, page, reference, dir, name) {
    const ours = await print(browser, html, page, dir, name);
    const [wordPages, htmlPages] = await Promise.all([rasterize(reference), rasterize(ours)]);
    const pixels = comparePages(wordPages, htmlPages, dir, name);
    const words = compareWords(await words_(reference), await words_(ours));

    return { name, wordPages: wordPages.length, htmlPages: htmlPages.length, pixelSimilarity: pixels, ...words };
}
...
html, body { margin: 0; padding: 0; }
    ...
    writeFileSync(file, html.replace('</head>', `${setup}\n</head>`));
    ...
    await page.close();
    writeFileSync(join(dir, `${name}.html.pdf`), pdf);
...
function comparePages(word, html, dir, name) {
    ...
    for (let index = 0; index < count; index++) {
        ...
        writeFileSync(join(dir, `${name}-p${index + 1}-html.png`), toPng(right, width, height));
    }
