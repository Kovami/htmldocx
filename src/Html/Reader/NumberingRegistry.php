...
final class NumberingRegistry
{
     ...
     */
    public function register(string $listStyleType, int $level, int $start, int $indentLeft, int $hanging, string $suffix = 'tab', bool $symbolBullet = false, ?int $markerTab = null, string $alignment = 'left'): int
    {
        ...
        for ($i = 0; $i <= 8; $i++) {
            ...
            $levels[] = $i === $level
                ? new ListLevel($i, $format, $text, max(0, $start), $indentLeft, $hanging, $suffix, $symbolBullet, $markerTab, $alignment)
